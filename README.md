...
- **Public Jobs Search Endpoints**: Expanded the public `GET /jobs` endpoint to support dynamic, case-insensitive query parameter searches (`?search=query`), matching against job titles, locations, and company names. Returns search results wrapped in global response interceptors for dynamic search dropdown auto-completion on frontends.
